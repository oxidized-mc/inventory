...
///
/// ```
/// use oxidized_inventory::DataComponentPatch;
///
...
impl ItemStack {
    ...
    ///
    /// Used by `PlayerInventory::suitable_hotbar_slot` to prefer replacing
    /// non-enchanted items when all hotbar slots are occupied.
...
pub fn max_stack_size(item: &ItemId) -> i32 {
    crate::item_ids::max_stack_size_by_name(&item.0)
}
